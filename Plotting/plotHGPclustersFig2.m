            ...
            for ii = 1:size(sigBins,1)
                if sigBins(ii,1) >= timeLims(colB,1) && sigBins(ii,2) <= timeLims(colB,2)
                    if row==1
                        ...
                        plot(sigBins(ii,:),yLimits{row}(2)*0.95*[1 1],'linewidth',2,'color',0.3*ones(3,1))
                        plot(mean(sigBins(ii,:)),yLimits{row}(2)*0.95,'*','linewidth',2,'color',0.1*ones(3,1))
                    end
                end
            end
